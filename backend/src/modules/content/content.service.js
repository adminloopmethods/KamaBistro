...
export const getAllWebpagesService = async () => {
  return await prismaClient.webpage.findMany({
    orderBy: [
      {createdAt: "asc"}, // or 'asc' depending on your preference
      {name: "asc"}, // secondary ordering by name
    ],
    include: {
      editor: true,
      verifier: true,
...
export const findWebpageIdByRouteService = async (
  route,
  locationName,
  activeOnly = false
) => {
  ...
  const whereClause = {
    route,
    ...(activeOnly && {Status: true}), // Only include active webpages when activeOnly is true
  };
...
// ---------------- ACTIVATE WEBPAGE ----------------
export const activateWebpageService = async (id) => {
  return await prismaClient.webpage.update({
    where: {id},
    data: {Status: true},
    include: {
      contents: {
...
// ---------------- DEACTIVATE WEBPAGE ----------------
export const deactivateWebpageService = async (id) => {
  return await prismaClient.webpage.update({
    where: {id},
    data: {Status: false},
    include: {
...
export const toggleWebpageStatusService = async (id) => {
  const webpage = await prismaClient.webpage.findUnique({
    where: {id},
    select: {Status: true},
  });
...
// ---------------- GET ACTIVE WEBPAGES ONLY (for public routes) ----------------
export const getActiveWebpagesService = async () => {
  return await prismaClient.webpage.findMany({
    where: {Status: true},
    include: {
      editor: true,
